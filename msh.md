...
fmod MODULE-DATABASE is
  ...
  sorts ModuleEnv ModuleConstruction .
  sorts Command{=>Modules} .

  var H : Header . vars ME ME' : ModuleExpression . var MOD : Module .
  var MC : ModuleConstruction . var MENV : ModuleEnv . var ID : Qid .
  var IL : ImportList . var S : Sort . var SS : SortSet . var SSDS : SubsortDeclSet .
  var OPDS : OpDeclSet . var MAS : MembAxSet . var EQS : EquationSet . var RLS : RuleSet .

  op .ModuleEnv : -> ModuleEnv .
  op _|->_ : Header ModuleExpression -> ModuleEnv [ctor] .
  op __ : ModuleEnv ModuleEnv -> ModuleEnv [ctor assoc comm id: .ModuleEnv prec 120] .
  ------------------------------------------------------------------------------------

  op _[_] : Header ModuleEnv -> Module .
  --------------------------------------
  eq H [ H |-> ME MENV ] = #upModule(ME) .
  ...
  op load-module : Header -> Command{=>Modules} .
  -----------------------------------------------
  eq load-module(H) = set-module(H, H) .

  op set-module : Header ModuleExpression -> Command{=>Modules} .
  ---------------------------------------------------------------
  eq set-module(H, ME) [ H |-> ME' MENV ] = H |-> ME MENV .
  eq set-module(H, ME) [           MENV ] = H |-> ME MENV [owise] .

  op List : Sort -> ModuleConstruction [ctor] .
  op Set  : Sort -> ModuleConstruction [ctor] .
  ...
  op _deriving_ : ModuleExpression ModuleConstruction -> ModuleExpression [ctor] .
  op _deriving_ : Module ModuleConstruction -> Module .
  -----------------------------------------------------
