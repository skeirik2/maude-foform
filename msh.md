...
MSH (for "Maude Shell") is a simple soup of state with one cell (the "strategy"
cell) which contains the program to be executed. Using sorts, you specify which
where to load arguments for commands from, and MSH does the plumbing for you.
...
fmod MSH is
  ...
  eq strategy < CMS                                     ; P > modules < MENV >
   = strategy < CMS < #resolveModule('STATE [ MENV ]) > ; P > modules < MENV > .
  ...
  eq strategy < CMST                                              ; P > modules < MENV > state < CTermS >
   = strategy < CMST < #resolveModule('TRACE [ MENV ]) , CTermS > ; P > modules < MENV > state < CTermS > .
endfm
```
