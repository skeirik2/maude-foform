...
fmod MODULE-TEMPLATE is
  ...
  vars H H' : Header . var I : Import . vars IL IL' IL'' : ImportList . vars S S' : Sort . vars SS SS' : SortSet . vars SSDS SSDS' : SubsortDeclSet .
  vars OPDS OPDS' : OpDeclSet . vars MAS MAS' : MembAxSet . vars EQS EQS' : EquationSet . vars RLS RLS' : RuleSet .
  var AS : AttrSet . vars A A' : Attr . var EQC : EqCondition . var C : Condition . vars MT MT' : ModuleTemplate .
  var MOD : Module . var Q : Qid . var TL : TypeList . var TYPE : Type . vars T T' : Term .
  vars SU SU' : Substitution . var SUBSTS : SubstitutionSet .
  var ST : SortTemplate . var SST : SubsortTemplate . var MAT : MembAxTemplate . var EQT : EqTemplate . var RLT : RuleTemplate .
  ...
  eq #resolveNames(ST  SSDS)   = #resolveNames(ST)   #resolveSubsorts(SSDS) .
  eq #resolveNames(SST OPDS)   = #resolveNames(SST)  #resolveOps(OPDS) .
  eq #resolveNames(SST MAS)    = #resolveNames(SST)  #resolveMemberships(MAS) .
  eq #resolveNames(MAT EQS)    = #resolveNames(MAT)  #resolveEquations(EQS) .
  eq #resolveNames(EQT RLS)    = #resolveNames(EQT)  #resolveRules(RLS) .
  eq #resolveNames(IL RLT)     = #resolveImports(IL) #resolveNames(RLT) .
  ...
  op #resolveEqCondition : EqCondition -> EqCondition .
  ...
  eq #resolveMemberships(cmb T : S if EQC [AS] .) = (cmb #resolveTerm(T) : #resolveSorts(S) if #resolveEqCondition(EQC) [#resolveAttrs(AS)] .) .
  ...
  eq #resolveEquations(ceq T = T' if EQC [AS] .) = (ceq #resolveTerm(T) = #resolveTerm(T') if #resolveEqCondition(EQC) [#resolveAttrs(AS)] .) .
  ...
  eq #resolveImports(IL I IL' I)       = #resolveImports(IL I IL') .
  eq #resolveImports(IL I IL' I IL'')  = #resolveImports(IL I IL' IL'') .
endfm
```
...
```{.maude .module-exp}
  ...
  op MetaTerms : -> ModFreeConstruction .
  ---------------------------------------
  ceq MetaTerms < Q > = forall ( sorts svar('X) . )
