...
-   `subsort : Sort Sort -> UniversalConstruction` takes two sort templates and
    adds the subsort relationship to any pair of sorts which match
    the templates.
...
```{.maude .univ}
  ...
  op univ-subsorts_. : SortPoset -> UniversalConstruction .
  ----------------------------------------------------
  eq univ-subsorts SPS . = forall ( sorts sortsFromPoset(SPS) . )
                           exists ( sorts none . )
                                  ( subsorts SPS . ) .
...
```{.maude .univ}
  ...
  eq META-TERM < Q > =   exists ( extending 'META-LEVEL . )
                                ( sorts 'Constant{Q} ; 'Variable{Q} ; 'GroundTerm{Q} ; 'Term{Q} . )
                       ; forall ( sorts svar('X) . )
                       ...
                       ; ( univ-subsorts 'Constant{svar('X)} < 'GroundTerm{svar('X)} < 'Term{svar('X)} . )
                       ; ( univ-subsorts 'Variable{svar('X)} < 'Term{svar('X)} . ) .
