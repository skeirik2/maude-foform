...
---   ceq joint-sort(S, MDS, MDS') = S if S inS intersect(MDS, MDS') .
---   ceq joint-sort(S, MDS, MDS') = maximalSorts(MOD, kind(getSorts(MOD)))
---                                if S inS MDS /\ (not S inS intersect(MDS, MDS')) .
---                                /\ MOD  := fromTemplate('X, intersect(connected-component(MDS, (sorts S .)), connected-component(MDS', (sorts S .))))
---                                /\ MOD' := 

  op joint-sort : Sort Module           Module           -> Sort .
  op joint-sort : Sort ModuleExpression ModuleExpression -> Sort .
  ----------------------------------------------------------------
  eq  joint-sort(S, ME,  ME')  = joint-sort(S, upModule(ME /\ ME', true), upModule(ME /\ ME', true)) [label e1] .
  eq  joint-sort(S, MOD, MOD') = joint-sort(S, asTemplate(MOD), asTemplate(MOD')) [label e1] .
